...
export async function generateMetadata({params}: {params: Promise<{locale: string}>}) {
  ...
  const t = await getTranslations({locale, namespace: 'HomeB'});
  return {
    title: t('meta.title'),
...
export default async function HomeB({params}: {params: Promise<{locale: string}>}) {
  const {locale} = await params;
  const t = await getTranslations({locale, namespace: 'HomeB'});
  ...
  return (
    <>
      ...
      <main className="flex-grow pt-20">
        {/* HERO — REGULATORY COUNTDOWN */}
        <Section variant="dark" spacing="md" className="relative overflow-hidden">
          <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-brand-surface -z-10 blur-3xl opacity-40 translate-x-1/2 -translate-y-1/2 rounded-full" />
          
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-3 border border-primary/30 text-primary px-4 py-2 mb-10">
        ...
        <Section variant="gray" spacing="md">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {['swissdamed', 'aml', 'leta', 'gpai'].map((key) => (
                <div key={key} className="bg-brand-bg p-6 border border-brand-border hover:border-primary premium-transition">
                  <div className="text-[9px] font-bold text-primary uppercase tracking-[0.2em] mb-4">{t(`regulatory.items.${key}.deadline`)}</div>
                  <h3 className="text-lg font-bold text-text-primary mb-3 leading-snug">{t(`regulatory.items.${key}.title`)}</h3>
                  <p className="text-text-muted text-sm leading-relaxed">{t(`regulatory.items.${key}.desc`)}</p>
                </div>
              ))}
            </div>
            <p className="text-text-muted text-sm mt-8 leading-relaxed max-w-2xl">
              {t('regulatory.note')}
            </p>
          </div>
        </Section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-16">
              {['governance', 'confidentiality', 'pragmatism'].map((key) => (
                <div key={key} className="border-l-2 border-brand-border pl-8 hover:border-primary premium-transition">
                  <h3 className="text-2xl font-bold mb-6 uppercase tracking-tighter text-text-primary">{t(`valueProps.items.${key}.title`)}</h3>
                  <p className="text-text-muted text-base leading-relaxed">
                    {t(`valueProps.items.${key}.desc`)}
                  </p>
                </div>
              ))}
            </div>
